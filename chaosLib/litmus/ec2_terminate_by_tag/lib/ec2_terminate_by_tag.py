...
def PrepareChaos(experimentsDetails, clients , resultDetails , eventsDetails , chaosDetails):
	...
	EC2InstanceTagList = experimentsDetails.InstanceTag
	#print(EC2InstanceIdList)
	#print(experimentsDetails)
	if len(EC2InstanceTagList) == 0:
		logging.error("no instance  found to terminate ")
	
	

 	# mode for chaos injection
	if experimentsDetails.Sequence.lower() == "serial":
		err = injectChaosInSerialMode(experimentsDetails , EC2InstanceTagList, chaosDetails , eventsDetails , resultDetails, clients, awslib, awslib1)
		if err != None:
			return err
	elif experimentsDetails.Sequence.lower() == "parallel":
		err = injectChaosInParallelMode(experimentsDetails , EC2InstanceTagList, chaosDetails , eventsDetails , resultDetails, clients, awslib, awslib1)
		if err != None:
			return err
	else:
...
def injectChaosInSerialMode(experimentsDetails , EC2InstanceTagList, chaosDetails , eventsDetails , resultDetails, clients, awslib, awslib1): 
	...
	while duration < experimentsDetails.ChaosDuration:
		...
		logging.error("Target EC2Instance list, %s", EC2InstanceTagList)
		...
		for i in range(EC2InstanceTagList):
